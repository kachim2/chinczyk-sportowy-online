...
int main(void) {
  ...
  while (!WindowShouldClose()) {
    ...
    if (!sdata->done_main) {
      movepawn(&pawns[sdata->MovePlayerId][sdata->MovePawnId], sdata->Movement,
               sdata->MovePlayerId);
    ...
    for (int i = 0; i < 13; i++) {
      for (int j = 0; j < 13; j++) {
        DrawRectangle(i * tilesizex + tilesizex / 20,
                      j * tilesizey + tilesizey / 20, (tilesizex * 9) / 10,
                      (tilesizey * 9) / 10, tilemap[i][j]._color);
      }
    }
    for (int i = 0; i < 4; i++) {
      for (int j = 0; j < 4; j++) {
        DrawEllipse(pawns[i][j].x * tilesizex + tilesizex / 2,
                    pawns[i][j].y * tilesizey + tilesizey / 2, tilesizex / 2.5,
                    tilesizey / 2.5, BLACK);
        DrawEllipse(pawns[i][j].x * tilesizex + tilesizex / 2,
                    pawns[i][j].y * tilesizey + tilesizey / 2, tilesizex / 3,
                    tilesizey / 3, pawns[i][j]._color);
      }
    }
    DrawText((to_string(sdata->DiceRoll) + "    " +
              to_string(sdata->MyPlayerId + 1) + "    " +
              to_string(sdata->selecting))
                 .c_str(),
             0, 0, 72, BLACK);
    EndDrawing();
  }
